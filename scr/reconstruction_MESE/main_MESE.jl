...
niM0 = NIVolume(T2_map[:,:,:,1],voxel_size=(acq.fov./acq.encodingSize))
...
# Reconstruire le chemin pour écrire dans "anat/" avec le bon nom de fichier
output_file = joinpath(dirname(path_out), "$(subject_session_name)_echo-1_MESE.nii.gz")
output_T2map = joinpath(dirname(path_out), "$(subject_session_name)_T2map.nii.gz")
output_M0map = joinpath(dirname(path_out), "$(subject_session_name)_M0map.nii.gz")

# Écriture du fichier
niwrite(output_file, ni)
niwrite(output_T2map,niT2)
niwrite(output_M0map,niM0)
# write nifti + BIDS
